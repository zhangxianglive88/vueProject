      ...
      <div class="mui-card-content">
        <div class="mui-card-content-inner">
          ...
          <p>
            ...
            <!-- 1. 经过分析发现，俺妞妞属于goodsinfo页面，数字属于numberbox组件 -->
            <!-- 2. 由于涉及到了父子组件的嵌套，所以，无法直接在goodsinfo页面中获取到选中的商品数量值 -->
            <!-- 3. 涉及到了子组件向父组件处传值了（事件调用机制） -->
            <!-- 4. 事件调用的本质：父向子传递方法，子调用这个方法，同时将数据当做参数传递给这个方法 -->
          </p>
        </div>
      </div>
    </div>
...
export default {
  data() {
    return {
      id: parseInt(this.$route.params.id), // 将路由参数对象中的id挂载到data上，方便后期调用
      lunbotuList: [], // 轮播图的数据
      goodsinfo: [], // 获取到的商品的信息
